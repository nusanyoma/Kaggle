...
class Generalizer:
    ...
    def load_partial(name):
        return np.load(Generalizer._partial_path(name))

    @staticmethod
    def load_whole(name):
        return np.load(Generalizer._whole_path(name))

    @staticmethod
    def save_partial(name, prediction):
        np.save(Generalizer._partial_path(name), prediction)

    @staticmethod
    def save_whole(name, prediction):
        np.save(Generalizer._whole_path(name), prediction)
